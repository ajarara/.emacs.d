...
Most of the stuff I install is through Quelpa. Quelpa pulls down a local version of the package I'm interested in, and installs it from there. This allows me to select individual branches, prune commits, and even browse logs. But I still have load in the package manager anyway.
...
Just taking this down, no need to commit.
#+BEGIN_SRC debug
error in process sentinel: elpy-rpc--default-error-callback: peculiar error: "segmentation fault (core dumped)"
error in process sentinel: peculiar error: "segmentation fault (core dumped)"
#+END_SRC
...
* nix
Soon enough I'll put up my working nix configuration.
#+BEGIN_SRC emacs-lisp :tangle init.el
