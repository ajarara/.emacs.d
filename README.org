#+STARTIP: overview
# Local variables:
# eval: (evil-set-register ?e "#+BEGIN_SRC emacs-lisp :tangle init.el")
# eval: (evil-set-register ?a "#+BEGIN_SRC emacs-lisp :tangle init.el")
# eval: (evil-set-register ?d "#+BEGIN_SRC emacs-lisp :tangle init.el")
...
and that's it! for more info, check out http://orgmode.org/manual/Template-elements.html#Template-elements
...
Let's try ansi-term instead. This config isn't tangled anymore (and will likely be removed after this commit.)
#+BEGIN_SRC emacs-lisp
(use-package term 
  :disabled t
...
okay, hopefull ansi-term plays nicer. I was under the impression that they were one and the same, but clearly, they're not. One difference immediately is calling ansi-term again creates a new terminal buffer. Another is that C-x is not sent to the process, or C-c. That's enough to make me start liking it.
#+BEGIN_SRC emacs-lisp :tangle init.el
...
Now I don't have to worry about swapping C-p -> C-h in every single corner of emacs. (A problem with this is that I still have to use 'p' in place of 'h' in modes like dired or magit commit logs.)
#+BEGIN_SRC emacs-lisp :tangle init.el
