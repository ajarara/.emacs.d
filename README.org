...
** lsp-mode
The language server protocol is neat! I'd like to be able to use emacs to edit java, kotlin code with little fuss. This requires jump to definition, find
usages, _maybe_ incremental compilation. 
...
** direnv
Only very slightly related to dired -- integrate emacs with direnv. Using this with lorri for rust/nix/cargo.el integration.
#+BEGIN_SRC emacs-lisp :tangle init.el
...
** vterm
Let's try this: there's some words of encouragement on the emacs subreddit. Only thing I want to change for now is closing the buffer on exit  

There's one minor catch here - on one computer I use NixOS, which manages vterm installation (on Mac you have to have libtool + cmake to install)
#+BEGIN_SRC emacs-lisp :tangle init.el
...
** password-store
I have a pretty sweet password store setup - I sync gpg public keys between all the places I use pass, and sync the store itself through the same mechanism.

This means I have lastpass functionality. This package brings dmenu like search to all computer platforms I use, and is written by 
#+BEGIN_SRC emacs-lisp :tangle init.el
