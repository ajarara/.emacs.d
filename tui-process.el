...
;; hmm..
;; we want to stash the process so that we know not to recreate the process
;; if it's already fired
;; and we want to invalidate if dependencies have changed
;; and tear down as we invalidate

;; it's likely possible to infer non-special dependencies if we go through the trouble of writing a macro
;; but react developers are used to defining dependencies, so we can defer that

(require 'tui)
(require 'cl-generic)

;; command creator takes a reference to a component and may return a command
...
(defun tui-process--execute (command stdout-filter stderr-filter)
  (let* ((stderr-buf (get-buffer-create (symbol-name (gensym " anonymous-stderr-buffer-"))))
         (process (make-process
                    :name (format "tui-process-%s" (prin1-to-string command))
                    :command command
                    :stderr stderr-buf
...
(tui-defun-2 my-component (&this this)
  ...
  (if-let ((proc (tui-process-create this :example (lambda () '("logger" "-s" "'please clap'")) nil)))
    (tui-div
